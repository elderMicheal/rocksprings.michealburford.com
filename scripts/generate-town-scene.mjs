...
for (const [name, baseColorFactor] of Object.entries(materialsDef)) {
  ...
  gltf.materials.push({
    name: `MAT_${name}`,
    pbrMetallicRoughness: { baseColorFactor, metallicFactor: name === "industrial" ? 0.2 : 0.02, roughnessFactor: 0.88 },
    ...(baseColorFactor[3] < 1 ? { alphaMode: "BLEND", doubleSided: true } : {}),
  });
...
function addBox(parent, name, material, translation, scale, extras = {}) {
  ...
  const i = addNode({ name, mesh, translation, scale, ...(Object.keys(extras).length ? { extras } : {}) });
  child(parent, i);
  return i;
}
function groupForLandmark(id) {
  ...
  if (["chalmers-property"].includes(id)) return "Chalmers_Farm";
  if (["old-ruins"].includes(id)) return "Ruins";
  ...
  if (["river"].includes(id)) return "River";
  return "Downtown";
}
...
function addVisual(parent, landmark) {
  ...
  if (kind === "industrial") {
    ...
    addBox(parent, `PRESENTATION_${landmark.id}_Stack`, "industrial", [6*horizontalScale,12,-4*horizontalScale], toWorldScale([2.2,24,2.2]));
    return;
  }
  if (kind === "ruins") {
    addBox(parent, "PRESENTATION_Ruins_Block_A", "stone", [-2*horizontalScale,1.5,0], toWorldScale([3,3,5]));
    addBox(parent, "PRESENTATION_Ruins_Block_B", "stone", [2*horizontalScale,1.0,1*horizontalScale], toWorldScale([2,2,4]));
    return;
  }
  if (kind === "river") {
    const centerX = (plan.bounds.minX + plan.bounds.maxX) / 2;
    const width = plan.bounds.maxX - plan.bounds.minX;
    addBox(parent, "PRESENTATION_River_Surface", "water", [(centerX-landmark.position[0])*horizontalScale,0,0], toWorldScale([width,0.4,15]));
    return;
  }
  const size = visual.size ?? [3,3,3];
  const height = size[1];
  addBox(parent, `PRESENTATION_${landmark.id}_Volume`, visual.material ?? "stone", [0,height/2,0], toWorldScale(size));
}

for (const landmark of plan.landmarks) {
  const parentGroup = groupIndex.get(groupForLandmark(landmark.id));
  ...
  child(parentGroup, semantic);
  namedNode.set(landmark.nodeName, semantic);
  addVisual(semantic, landmark);
  const ref = addNode({ name: `LANDMARK_REF_${landmark.id.replaceAll("-","_")}`, translation: toWorld(landmark.position),  });
  child(groupIndex.get("Landmarks"), ref);
}

function roadPosition(road) {
  if (road.landmarkId) return landmarkById.get(road.landmarkId).position;
  return road.position;
}
for (const road of plan.roads) {
  const pos = roadPosition(road);
  let semantic = namedNode.get(road.nodeName);
  if (semantic === undefined) {
    semantic = addNode({ name: road.nodeName, translation: toWorld(pos), children: [], extras: { classification: road.classification } });
    child(groupIndex.get(road.id === "jefferson-street" ? "Southside_Industrial" : "Residential"), semantic);
    namedNode.set(road.nodeName, semantic);
  }
...
for (const route of plan.routes) {
  const routeParent = addNode({ name: route.name, children: [],  });
  child(groupIndex.get("Authored_Paths"), routeParent);
  const worldPoints = route.points.map(toWorld);
  const flat = new Float32Array(worldPoints.flat());
  const view = addBuffer(Buffer.from(flat.buffer), 34962);
  const mins = [0,1,2].map((axis) => Math.min(...worldPoints.map((p) => p[axis])));
  const maxs = [0,1,2].map((axis) => Math.max(...worldPoints.map((p) => p[axis])));
  const accessor = addAccessor(view,5126,worldPoints.length,"VEC3",{min:mins,max:maxs});
  const mesh = gltf.meshes.length;
  gltf.meshes.push({ name: `MESH_${route.name}`, primitives: [{ attributes: { POSITION: accessor }, material: materialIndex.get("route"), mode: 3 }] });
  const line = addNode({ name: `PRESENTATION_${route.name}_Line`, mesh });
  child(routeParent,line);
  worldPoints.forEach((point,index) => {
    const wp = addNode({ name: `${route.name}_Waypoint_${String(index+1).padStart(2,"0")}`, translation: point });
    child(routeParent,wp);
  });
}
...
for (const camera of plan.cameras) {
  ...
  const node = addNode({ name: camera.name, camera: cameraIndex, translation: toWorld(camera.position), extras: { target: toWorld(camera.target) } });
  child(groupIndex.get("Authored_Cameras"),node);
}
...
function jsonChunkBuffer(value) {
  const raw = Buffer.from(JSON.stringify(value));
  const pad = (4 - (raw.length % 4)) % 4;
  return pad ? Buffer.concat([raw, Buffer.alloc(pad,0x20)]) : raw;
}
const jsonChunk = jsonChunkBuffer(gltf);
const binPad = (4-(binary.length%4))%4;
const binChunk = binPad ? Buffer.concat([binary,Buffer.alloc(binPad)]) : binary;
...
const manifest={
  schemaVersion:1,
  id:plan.id,
  title:"Rock Springs: Jackie's Window",
  model:{url:"/assets/scenes/jackies-window/rock-springs-jackies-window.glb",sha256:createHash("sha256").update(glb).digest("hex"),bytes:glb.length,format:"glTF 2.0 binary",stats,extensions:[]},
  fallback:{kind:"neutral",alt:"Neutral background behind the interpretive scene"},
  provenance:{classification:"generated-presentation-media",sourceReferences:["Jackie's Window, Part 1, approved Chapters 1–8"],auditedAgainstRevision:plan.source.writingRevision,spatialPlan:"scene-data/jackies-window-spatial-plan.json",evidenceDocument:"docs/phase-2/JACKIES_WINDOW_SPATIAL_EVIDENCE.md",generator:"scripts/generate-town-scene.mjs",sourceManifest:"public/assets/scenes/jackies-window/source-manifest.json"},
  canon:{geometry:"interpretive",distances:"source-stated distances are scaled; otherwise approximate",architecture:"interpretive",publicLandmarks:false,nodeNaming:"CANON_* identifies source-established locations or relationships; INFERRED_* identifies reasoned placement; PRESENTATION_* identifies connective geometry.",notice:"The scene is an interpretive visualization of the approved Part One writing. Unspecified distances, coordinates, bearings, and architecture are not asserted as canon."},
  requiredLandmarkNodes:plan.landmarks.map((x)=>x.nodeName),
  authoredViews:plan.cameras.filter((x)=>!x.name.startsWith("Camera_Descent_")).map((x)=>({cameraNode:x.name,label:x.label,sourceReferences:x.source})),
  descent:{startCameraNode:"Camera_Descent_Start",endCameraNode:"Camera_Descent_End"},
  authoredRoutes:plan.routes.map((x)=>x.name),
  anchors:[]
};

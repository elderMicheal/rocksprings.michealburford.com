...
and AI agent. Read it before changing deployment, DNS, hosting, CI, release
configuration, public content behavior, or generated application state.
...
## Content and synchronization contract

For any work that changes authored-content admission, public routes, indexes,
reader behavior, map evidence, generated content, or presentation structure:

1. Read the current root `README.md`.
2. Read `docs/AUTHORING_FOR_PUBLICATION.md` when author metadata, slugs,
   publication intent, or new source kinds are involved.
3. Read `docs/CONTENT_SYNC.md` and follow its source-of-truth and validation
   rules.
4. Read the relevant specialized contract under `docs/phase-2/`.
5. Inspect the current branch and generated package before using historical
   plans, old screenshots, or prior conversation state as implementation truth.

The Writing repository remains authoritative for manuscript prose and canonical
writing. This repository owns explicit public approval, deterministic export,
presentation, routing, generated public artifacts, and deployment.

Reader publication and map evidence are separate trust boundaries. Map evidence
may use pinned, paraphrased facts from canonical unpublished writing under the
documented evidence policy; that does not publish the source prose.

Generated files must be regenerated by their owning scripts, not hand-edited to
force agreement.

Do not solve a new story, anthology, book, or top-level work by adding
title-specific route logic or hard-coded prose when the application lacks a
generic content model for that source kind. Extend the shared model and update
the author-facing contract in the same change.

Documentation and code must describe the same behavior. If they disagree, stop
and resolve the discrepancy before building further work on top of it.

Use the smallest local validation tier that proves the requested change, as
defined in `docs/CONTENT_SYNC.md`. Full release validation remains mandatory
before deployment; reduced local testing is not permission to weaken the release
gate.
...
custom domain resolves to Cloudflare, and the owner has not lost unrelated local
or remote work. If any of those conditions is unknown, report the unknown
